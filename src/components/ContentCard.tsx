...
interface ContentCardProps {
  ...
  color: 'lime' | 'pink' | 'amber' | 'teal' | 'emerald' | 'red' | 'cyan' | 'orange' | 'neutral';
...
const colorClasses = {
  ...
  neutral: 'bg-transparent hover:bg-amber-50',
};
...
const ContentCard: React.FC<ContentCardProps> = ({
  ...
  const articleElement = (
    ...
    >
      <header className="flex items-center border-b border-stone-900">
        <div
          className="h-auto inline-flex self-stretch items-center justify-center p-4 border-r border-stone-900"
          aria-hidden="true"
        >
          {icon}
        </div>
        <h2 className="p-4 text-stone-900 text-2xl font-sans font-medium">{title}</h2>
      </header>
  ...
  if (to) {
    return (
      <Link href={to} target={target || '_blank'} className="w-full">
        {articleElement}
      </Link>
    );
  }

  return <div className="w-full">{articleElement}</div>;
};
